                         ...
                         std::map<Z2i::SCell,double>& weightMap)
{
...

    {
        ...
        for (GluedCurveIteratorPair it = gcRange.begin(); it != gcRange.end(); ++it) {
            if(it->first.connectorType()==makeConvex) continue;
    ...
    std::vector<double> tangentWeightVector;
    {
        ...
        int i = 0;
        do {
            if(it->first.connectorType()==makeConvex){
                ++it;
                continue;
            }
...
int main(){
    ...
    std::string imgPath = "../images/flow-evolution/problematic_3.pgm";
    SegCut::Image2D image = GenericReader<SegCut::Image2D>::import(imgPath);

    std::string outImageFolder = "issueout/problematic_3";
    std::string cutOutputPath;
    std::string imageOutputPath;


    KSpace KImage;
    setKImage("../images/flow-evolution/problematic_3.pgm",KImage);
